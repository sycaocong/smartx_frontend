...
import { ArrowUpRight, AlertCircle, Check, RefreshCw, Shield } from 'lucide-react'
...
export function WithdrawPage() {
  ...
  const { balances, isLoading, error, clearError, submitWithdraw } = useWalletStore()
  ...
  const fee = asset === 'BTC' ? 0.0005 : asset === 'ETH' ? 0.005 : 1

  const handleSubmit = async () => {
    if (!asset || !address || !amount || parseFloat(amount) <= 0 || !agreed || !privateKey) return
    if (parseFloat(amount) > availableBalance) {
      clearError()
      return
    }
    
    const success = await submitWithdraw(DEFAULT_CHAIN_ID, asset, fromAddress, address, amount, privateKey)
    if (success) {
      setShowSuccess(true)
      setTimeout(() => {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="card p-6">
        <h2 className="text-text-primary font-semibold text-lg mb-6">
          {language === 'zh' ? '提现信息' : 'Withdrawal Information'}
        </h2>

        {showSuccess && (
          <div className="mb-6 p-4 bg-green-bg text-green rounded-lg flex items-center gap-2">
            <Check className="w-5 h-5" />
            {language === 'zh' ? '提现请求已提交，等待审核' : 'Withdrawal request submitted, awaiting review'}
          </div>
        )}
        ...
        <div className="space-y-6">
          ...
          <div>
            <label className="text-text-muted text-sm mb-2 block">
              {language === 'zh' ? '提现地址' : 'Withdraw Address'}
            </label>
            <input
              type="text"
              value={address}
              onChange={(e) => setAddress(e.target.value)}
              placeholder={language === 'zh' ? '请输入提现地址' : 'Enter withdrawal address'}
          ...
          {needsTag && (
            <div>
              ...
              <input
                ...
                placeholder={language === 'zh' ? '请输入Memo' : 'Enter memo'}
                className="input-field w-full"
                disabled={isLoading}
              />
            </div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {[0.25, 0.5, 0.75, 1].map((ratio) => (
              <button
                ...
                onClick={() => {
                  if (balance) {
                    const maxAmount = (parseFloat(balance.free) - fee) * ratio
          ...
          <div className="flex items-start gap-3 p-4 bg-primary/10 rounded-lg">
            ...
            <div className="text-text-secondary text-sm">
              ...
              <ul className="space-y-1">
                <li>• {language === 'zh' ? '请确认提现地址正确无误' : 'Please confirm the withdrawal address is correct'}</li>
                <li>• {language === 'zh' ? '提现前请确保地址支持该币种' : 'Ensure the address supports this asset before withdrawing'}</li>
                <li>• {language === 'zh' ? '提现手续费将从您的余额中扣除' : 'Withdrawal fee will be deducted from your balance'}</li>
                <li>• {language === 'zh' ? '提现处理时间取决于区块链网络状况' : 'Processing time depends on blockchain network conditions'}</li>
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="agreement" className="text-text-secondary text-sm">
              ...
              <a href="#" className="text-primary hover:underline ml-1">
                {language === 'zh' ? '用户协议' : 'User Agreement'}
              </a>
              {language === 'zh' ? '和' : ' and '}
          ...
          <button
            ...
            disabled={!address || !isValidAmount || !agreed || !privateKey || isLoading}
            className="w-full py-4 bg-gradient-to-r from-red to-red/80 text-bg-dark font-bold rounded-lg hover:shadow-lg hover:shadow-red/40 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2"
          >
            ...
            {isLoading ? (
              ...
              language === 'zh' ? '确认提现' : 'Confirm Withdrawal'
            )}
          </button>
        </div>
      </div>
